...
package it.unibo.sonar_data_receiver
...
class Sonar_data_receiver ( name: String, scope: CoroutineScope  ) : ActorBasicFsm( name, scope ){
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		return { //this:ActionBasciFsm
				state("state_init") { //this:State
					action { //it:State
						println("[SonarDataReceiver] Started.")
				...
				state("state_idle") { //this:State
					action { //it:State
						println("[SonarDataReceiver] Listening for sonar data...")
						 PrevAlarm = Alarm  
						//genTimer( actor, state )
					}
					//After Lenzi Aug2002
				...
				state("state_handle_sonar_data") { //this:State
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("sonar_data(DISTANCE,TIMESTAMP)"), Term.createTerm("sonarevent(DISTANCE,TIMESTAMP)"), 
								...
								println("[SonarDataReceiver] Received sonar data: ${payloadArg(0)}")
								if(  Alarm != PrevAlarm  
								 ){if(  Alarm == true  
								 ){emit("stop", "stop(_)" ) 
								}
