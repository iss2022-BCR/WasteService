...
class CoapObserver : CoapHandler {
    ...
    /**
     * Wait until a resource message is added to the COAP history.
     ...
     */
    fun waitNextEntry(refreshRate: Int = 300, timeout: Int = 1000): Boolean {
        ColorsOut.outappl("Waiting for next COAP entry", ColorsOut.BLUE)
        var numMessages = getCoapHistory().size
        ...
        while (timeoutCounter < tTimeout) {
            CommUtils.delay(rRefreshRate)
            if(tTimeout > 0)
                timeoutCounter += rRefreshRate

            if(getCoapHistory().size > numMessages)
                return true
        }
    ...
    fun waitForSpecificHistoryEntry(entry: String, refreshRate: Int = 300, timeout: Int = 1000): Boolean {
        ColorsOut.outappl("Waiting for COAP entry: $entry", ColorsOut.BLUE)
        var found = false
